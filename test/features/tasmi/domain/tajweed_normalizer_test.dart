...
void main() {
  ...
  group('TajweedNormalizer.compareWord', () {
    test('should return "correct" for exact match with different tashkeel', () {
      ...
      final result =
          TajweedNormalizer.compareWord(spoken: spoken, expected: expected);
      expect(result, WordMatchResult.correct);
    });

    test(
        'should return "closeError" for a tajweed mistake (similarity >= 0.75)',
        () {
      const spokenClose = 'العلمين';
      const expectedClose = 'الْعَالَمِينَ'; // Normalized: العالمين
      final resultClose = TajweedNormalizer.compareWord(
          spoken: spokenClose, expected: expectedClose);
      expect(resultClose, WordMatchResult.closeError,
          reason: 'Similarity should be high');
    });
    ...
    test('should return "closeError" for another close mistake', () {
      ...
      final resultClose2 = TajweedNormalizer.compareWord(
          spoken: spokenClose2, expected: expectedClose2);
      expect(resultClose2, WordMatchResult.closeError,
          reason: "Similarity should be high for missing 'ال'");
    });

    test(
        'should return "wrongWord" for a completely different word (similarity < 0.75)',
        () {
      const spoken = 'اياك';
