  ...
  static Color getTajweedColor(String rule, QuranThemeMode mode) {
    ...
    if (rule.startsWith('m')) {
      return isDark
          ...
          : const Color(0xFFD32F2F); // Madd (Red)
    } else if (rule.startsWith('o')) {
      return isDark
          ? const Color(0xFFFF1744)
          : const Color(0xFFB71C1C); // Obligatory Madd (Darker Red)
    } else if (rule.startsWith('g')) {
      return isDark
          ? const Color(0xFF69F0AE)
          : const Color(0xFF2E7D32); // Ghunnah (Green)
    } else if (rule.startsWith('f')) {
      return isDark
          ? const Color(0xFFB9F6CA)
          : const Color(0xFF43A047); // Ikhfa (Lighter Green)
    } else if (rule.startsWith('q')) {
      return isDark
          ? const Color(0xFF40C4FF)
          : const Color(0xFF1976D2); // Qalqalah (Blue)
    } else if (rule.startsWith('p')) {
      return isDark
          ? const Color(0xFFFFD180)
          : const Color(0xFFE65100); // Qalb (Orange/Amber)
    } else if (rule.startsWith('c')) {
      return isDark
          ? const Color(0xFFEA80FC)
          : const Color(0xFF8E24AA); // Ikhfa Meem Saakin (Purple/Pink)
      ...
      return isDark
          ? Colors.white24
          : Colors.black26; // Silent letters (Faded Grey)
    }
