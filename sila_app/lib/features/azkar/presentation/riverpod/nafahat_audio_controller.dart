...
class NafahatPlayerState {
  ...
  Ibtihal? get currentIbtihal => filteredPlaylist.isNotEmpty && currentIndex < filteredPlaylist.length 
  ...
  NafahatPlayerState copyWith({
    ...
    return NafahatPlayerState(
      ...
      sleepTimerDuration: clearSleepTimer ? null : (sleepTimerDuration ?? this.sleepTimerDuration),
...
class NafahatAudioController extends _$NafahatAudioController {
  ...
  static const String _nakshabandiBaseUrl = 'https://archive.org/download/20240309_20240309_1714/';
  static const String _alToukhyBaseUrl = 'https://archive.org/download/Mohammed_Al_Tokhi_Ibtihalat/';
  ...
  static final _defaultPlaylist = [
    // Nakshabandi
    _createIbtihal('N01', 'إبتهال قبضة الله', '001  إبتهال قبضة الله الشيخ سيد النقشبندي.mp3', _nakshabandiBaseUrl, _nakshabandiName),
    _createIbtihal('N02', 'ابتهالات كاملة', '002  ابتهالات الشيخ النقشبندى كامله.mp3', _nakshabandiBaseUrl, _nakshabandiName),
    _createIbtihal('N03', 'إبتهالات نادرة (١٩٦٤)', '003  إبتهالات نادرة جداللشيخ سيد النقشبندي من ساحة مسجد السيدة زينب بالقاهرة عام 1964مبصوت رائع وجميل.mp3', _nakshabandiBaseUrl, _nakshabandiName),
    _createIbtihal('N04', 'يا رب إن عظمت ذنوبي', '004  الشيخ سيد النقشبندى, يا رب ان عظمت ذنوبى كثرة.mp3', _nakshabandiBaseUrl, _nakshabandiName),
    _createIbtihal('N05', 'النفس تشكو', '005  النقشبندي - ابتهال النفس تشكو - كامل - جودة عالية.mp3', _nakshabandiBaseUrl, _nakshabandiName),
    _createIbtihal('N08', 'مولاي إني ببابك', '008  مولاي إني ببابك قد بسطت يدي   للشيخ النقشبندى.mp3', _nakshabandiBaseUrl, _nakshabandiName),
    _createIbtihal('N12', 'يا رب كرمك علينا', '012  يارب كرمك علينا.mp3', _nakshabandiBaseUrl, _nakshabandiName),
    _createIbtihal('N13', 'يا من له ستر علي', '013  يامن له ستر علي جميل  ابتهال الشيخ سيد النقشبندي_.mp3', _nakshabandiBaseUrl, _nakshabandiName),
    
    // Al-Toukhy
    Ibtihal(
      id: 'T1',
      title: 'ابتهال نادر: من لي سواك',
      url: 'https://archive.org/download/AshSheekhSeedAnNqshbndeeGlshKhastNadrtGdaMnMnzlhFtrtAsSb3eenyat/Ash-Sheekh%20Muhammad%20At-Tukhee%20abthal%20nadr%20mn%20lee%20swak%20mn%20slat%20fgr.mp3',
      artistName: 'محمد الطوخي',
    ),
  ];

  static Ibtihal _createIbtihal(String id, String title, String filename, String baseUrl, String artist) {
    return Ibtihal(
      id: id,
      title: title,
  ...
  NafahatPlayerState build() {
    ...
    _player.playerStateStream.listen((ps) {
      if (state.isLoading != (ps.processingState == ProcessingState.loading)) {
         state = state.copyWith(playerState: ps, isLoading: ps.processingState == ProcessingState.loading);
      } else {
  ...
  void previous() {
    if (state.filteredPlaylist.isEmpty) return;
    final prevIndex = (state.currentIndex - 1 + state.filteredPlaylist.length) % state.filteredPlaylist.length;
