...
final dailyReportProvider = Provider<AsyncValue<DailyReportState>>((ref) {
  ...
  return tracker.whenData(
    (value) => DailyReportState(today: value.today, yesterday: value.yesterday, isMale: value.isMale),
  );
});
...
final recordsRangeProvider = FutureProvider.family<List<IbadahRecord>, ({DateTime start, DateTime end})>((ref, range) async {
  final repo = await ref.watch(ibadahRepositoryProvider.future);
