...
class _SurahDetailPageState extends ConsumerState<SurahDetailPage> {
  ...
  PreferredSizeWidget _buildCustomAppBar(QuranSettings settings, String surahName, int juz) {
    ...
    return AppBar(
      ...
      actions: [
        IconButton(
          icon: Icon(_isBookmarked ? Icons.bookmark : Icons.bookmark_border, color: iconColor),
                        ...
                        decoration: BoxDecoration(
                          border: Border.symmetric(
                            horizontal: BorderSide(
                              color: QuranUIUtils.getAccentColor(settings.themeMode).withOpacity(0.2),
                              width: 1,
                            ),
                          ),
                        ),
  ...
  List<InlineSpan> _buildAyahSpans(int surahNum, int ayahNum,
    ...
    final tajweedSpans = QuranUIUtils.buildTajweedSpans(rawVerse, settings.themeMode,
  ...
  TextStyle _getQuranStyle(QuranSettings settings) {
    ...
    switch (settings.fontFamily) {
      case 'KFGQPCUthmanicScript':
        return TextStyle(fontFamily: 'KFGQPCUthmanicScript', fontSize: size, height: height, color: color);
      case 'Amiri':
        return GoogleFonts.amiri(fontSize: size, height: height, color: color);
      case 'Scheherazade':
        return GoogleFonts.scheherazadeNew(fontSize: size, height: height, color: color);
      case 'Noto Naskh':
        return GoogleFonts.notoNaskhArabic(fontSize: size, height: height, color: color);
      default:
        return GoogleFonts.getFont(settings.fontFamily, fontSize: size, height: height, color: color);
    }
  }
    ...
    showModalBottomSheet(
      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
            Text('reading_settings_title'.tr(), textAlign: TextAlign.center, style: GoogleFonts.cairo(fontSize: 20, fontWeight: FontWeight.bold, color: QuranUIUtils.getTextColor(settings.themeMode))),
            const SizedBox(height: 24),
            Row(children: [
              Expanded(child: Consumer(builder: (context, ref, child) {
                final reciterName = ref.watch(reciterControllerProvider).when(data: (r) => r.nameArabic, loading: () => 'loading_label'.tr(), error: (e, s) => 'reciter_label'.tr());
              ...
              Expanded(child: _buildSettingTile(icon: Icons.info_outline, label: 'about_surah_label'.tr(), onTap: () {}, settings: settings)),
            ]),
            ...
            const SizedBox(height: 24),
            Text('quran_font_label'.tr(), textAlign: TextAlign.center, style: GoogleFonts.cairo(fontSize: 14, fontWeight: FontWeight.bold, color: QuranUIUtils.getTextColor(settings.themeMode))),
            const SizedBox(height: 12),
            ...
            Consumer(builder: (context, ref, child) {
              ...
              return Row(children: [
                Icon(Icons.format_size, color: QuranUIUtils.getTextColor(currentSettings.themeMode)),
                Expanded(child: Slider(
                  value: currentSettings.fontSize.clamp(16.0, 44.0),
                  min: 16,
                  max: 44,
                  activeColor: QuranUIUtils.getAccentColor(currentSettings.themeMode),
  ...
  Widget _buildFontOption(WidgetRef ref, String fontFamily, String label, QuranSettings settings) {
    ...
    return GestureDetector(
      ...
      child: Container(
        padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
        decoration: BoxDecoration(
          color: isSelected ? QuranUIUtils.getAccentColor(settings.themeMode).withOpacity(0.1) : Colors.transparent,
          borderRadius: BorderRadius.circular(20),
          border: Border.all(color: isSelected ? QuranUIUtils.getAccentColor(settings.themeMode) : Colors.grey),
        ),
        child: Text(label, style: TextStyle(fontFamily: fontFamily, fontSize: 16, color: QuranUIUtils.getTextColor(settings.themeMode))),
      ),
    );
  }
